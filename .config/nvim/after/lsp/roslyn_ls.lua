...
local function refresh_diagnostics(client)
	-- 1. Consultar el estado real del cliente en el registro de Neovim
	local active_client = vim.lsp.get_client_by_id(client.id)

	-- 2. Manejo de error: Si el cliente fue destruido (ej. por :LspRestart),
	-- abortar silenciosamente la ejecución para evitar el crash.
	if not active_client then
		return
	end

	-- 3. Proceder con el cliente activo y validado
	for buf, _ in pairs(active_client.attached_buffers) do
